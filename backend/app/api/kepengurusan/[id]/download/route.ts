...
/**
 * Endpoint GET /api/kepengurusan/[id]/download
 * Menghasilkan Secure Signed URL (TTL 5 menit) untuk dokumen PDF SK
...
export async function GET(
    ...
    try {
        ...
        try {
            kepengurusan = await KepengurusanService.getById(id);
        } catch (err: any) {
            // .single() melempar PGRST116 saat baris tidak ada → 404.
            // Error lain (network/schema) jangan disembunyikan — biarkan jatuh ke 500.
            if (err?.code === "PGRST116") {
                return NextResponse.json(
                    { status: "fail", message: "data kepengurusan tidak ditemukan" },
                    { status: 404 }
                );
            }
            throw err;
        ...
        console.error("Gagal mengunduh dokumen SK:", error);
        return NextResponse.json(
            {
                status: "error",
